...
class _InfoState extends State<Info> {
  ...
  Widget buildsmth(User1 us) => Padding(
      padding: const EdgeInsets.only(left: 15, right: 15),
      child: Container(
          padding: const EdgeInsets.all(20),
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(10.0),
            border: Border.all(style: BorderStyle.solid, color: widget.color),
          ),
          child: Column(
            children: [
              TextField(
                ...
                StatefulBuilder(builder: (context, passState) {
                  return TextField(
                    ...
                    decoration: InputDecoration(
                      labelText:
                          'Nhập mật khẩu để xác nhận đổi tên username/email:',
            ...
            onPressed: () async {
              ...
              if (txtPass.text.isEmpty) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                    content:
                        Text('Vui lòng nhập mật khẩu để đổi username/email')));
              } else if (checkUser) {
                ScaffoldMessenger.of(context).clearSnackBars();
                Navigator.of(context).pop();
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                    content: Text(
                        'Username đã được dùng, vui lòng đổi sang tên khác')));
